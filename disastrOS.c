...
void timerInterrupt() {
  if (log_file)
    fprintf(log_file, "TIME: %d\tPID: %d\tACTION: %s\n", disastrOS_time,
            running->pid, "TIMER_OUT");
  ++disastrOS_time;
  printf("time: %d\n", disastrOS_time);
  internal_schedule();
  if (log_file)
    fprintf(log_file, "TIME: %d\tPID: %d\tACTION: %s\n", disastrOS_time,
            running->pid, "TIMER_IN");
...
void disastrOS_trap() {
  ...
  if (log_file)
    fprintf(log_file, "TIME: %d\tPID: %d\tACTION: %s %d\n", disastrOS_time,
            running->pid, "SYSCALL_IN", syscall_num);
...
return_to_process:
  if (log_file)
    fprintf(log_file, "TIME: %d\tPID: %d\tACTION: %s %d\n", disastrOS_time,
            running->pid, "SYSCALL_OUT", syscall_num);
  if (running)
    setcontext(&running->cpu_state);
  else {
...
void disastrOS_start(void (*f)(void*), void* f_args, char* logfile) {
  ...
  // message queue syscall hanlders
  syscall_vector[DSOS_CALL_MQOPEN] = internal_mqOpen;
  syscall_numarg[DSOS_CALL_MQOPEN] = 3;
  ...
  syscall_numarg[DSOS_CALL_MQREAD] = 4;

  syscall_vector[DSOS_CALL_MQWRITE] = internal_mqWrite;
  syscall_numarg[DSOS_CALL_MQWRITE] = 4;

  syscall_vector[DSOS_CALL_MQUNLINK] = internal_mqUnlink;
  syscall_numarg[DSOS_CALL_MQUNLINK] = 1;
  ...
  sigaddset(&trap_context.uc_sigmask,
            SIGALRM);  //< this disables alarm during system trap
  trap_context.uc_stack.ss_flags = 0;
  trap_context.uc_link = &main_context;
  makecontext(&trap_context, disastrOS_trap,
              0);  //<< this extablishes a context for the system

  disastrOS_debug("setting entry point for timer interrupt... ");
  interrupt_context =
      trap_context;  // the interrupt and the system live on the same stack
  interrupt_context.uc_link = &main_context;
  sigemptyset(&interrupt_context.uc_sigmask);
  makecontext(&interrupt_context, timerInterrupt,
              0);  //< this is a context for the interrupt
...
int disastrOS_semOpen(int id, int flags, unsigned int count) {
  return disastrOS_syscall(DSOS_CALL_SEMOPEN, id, flags, count);
}
...
// messagequeues system calls

int disastrOS_mqOpen(int id, int flags, int priority) {
  return disastrOS_syscall(DSOS_CALL_MQOPEN, id, flags, priority);
}
...
int disastrOS_mqRead(int fd, char* buf, int buf_size, int* priority) {
  return disastrOS_syscall(DSOS_CALL_MQREAD, fd, buf, buf_size, priority);
}

int disastrOS_mqWrite(int fd, char* buf, int buf_size, int priority) {
  return disastrOS_syscall(DSOS_CALL_MQWRITE, fd, buf, buf_size, priority);
}

int disastrOS_mqUnlink(int id) {
  return disastrOS_syscall(DSOS_CALL_MQUNLINK, id);
}
